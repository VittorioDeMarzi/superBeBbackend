...
class PropertyControllerTest {
    ...
    @Test
    public void testGetPropertyById_returnPropertyWhenFound() {
        // (1) arrange
        when(mockPropertyService.findPublicPropertyDtoById(1L)).thenReturn(examplePropertyResponseDto);
        // (2) act
        ResponseEntity<PropertyResponseDto> result = propertyController.getPropertyById(1L);
        // (3) assert
        assertEquals(HttpStatus.OK, result.getStatusCode());
        assertEquals(examplePropertyResponseDto, result.getBody());
        verify(mockPropertyService, times(1)).findPublicPropertyDtoById(1L);
    }
